...
contract Passport is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, UUPSUpgradeable {
    ...
    uint256[] public fixed_MVERSE_TOKEN_AMOUNT_PASSPORTS; // Number of Passports Minted: [0] = Member_Level_1; [1] = Member_Level_2; [2] = Member_Level_3 ...
    ...
    function initialize(
        ...
        fixed_MVERSE_TOKEN_AMOUNT_PASSPORTS = _fixed_MVERSE_TOKEN_AMOUNT_PASSPORTS;
    ...
    function tokenExists(
        ...
        if (!checkOnlyPassports && id_TOKEN == id_MVERSE_TOKEN) // If param checkOnlyPassports is false, check if the Id specified is equal to the $MVerse Token Id
            return true; // return true because this Id is from the $MVerse Token

        // loop through Passport Ids
        for(uint256 i = 0; i < id_PASSPORTS.length; i++)
            if (id_PASSPORTS[i] == id_TOKEN) // if there is a Passport with the specified Id must return true
                return true; // return true because this Id is from one of the Passports
        ...
        public
    {
        ...
        require(tokenExists(ids[1], true), "Passport Id doesn't exist"); // ids[1] must be the Passport Id, so it needs to be in the range of 1..9
    ...
    function setFixedMVerseTokenAmount(
        ...
        require(tokenExists(id_PASSPORT, true), "Passport with this Id doesn't exist"); // Id of Passport must be different from zero and less or equal than the total Passports Ids
        //require(rate_PASSPORT != 0, "Rate must be different from zero");
        fixed_MVERSE_TOKEN_AMOUNT_PASSPORTS[id_PASSPORT-1] = fixed_MVERSE_TOKEN_AMOUNT_PASSPORT;
    }
    ...
    function getFixedMVerseTokenAmount(
        ...
        return fixed_MVERSE_TOKEN_AMOUNT_PASSPORTS[id_PASSPORT-1];
    }
